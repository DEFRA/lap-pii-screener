...
| -------------- | --------------- | --------------------------------------------------------------- |
| **CLI**        | `src/cli.py`    | Interactive use, CI/CD pipelines, scripted scans                |
| **MCP Server** | `src/server.py` | VS Code Copilot Chat integration via the Model Context Protocol |
...
flowchart TD
    subgraph EP["ENTRY POINTS"]
        direction LR
        CLI["<b>CLI</b> · src/cli.py<br/>scan · status · setup · report<br/>obfuscate · rollback · edit"]
        MCP["<b>MCP Server</b> · src/server.py<br/>scan_codebase · get_report · list_findings<br/>get_remediation · check_scanner_status"]
    end
...
1. Calls `detect_available_scanners()` to determine which requested scanner backends are available and assigns a **tier** (1 = baseline scanners, 2 = SonarQube available, 3 = SonarCloud configured).
...
6. Builds a `Report`, triggers `report.build_summary()`, and caches it for later report and MCP reuse.
...
Loads `src/config/regulations.yaml` and builds an inverted index from PII category to applicable regulations (UK GDPR, PCI DSS, PSR 2017). Called during report assembly to populate `Finding.regulations`.
...
duration_seconds : float
  tier_used        : int         — 1=baseline, 2=SonarQube, 3=SonarCloud
...
```mermaid
flowchart TD
    CLI["<b>CLI</b> · src/cli.py<br/>sensitive-scanner scan"]
    MCP["<b>MCP Server</b> · src/server.py<br/>scan_codebase tool"]
...
| 3     | `src/config/suppress.txt` (repository default) | Organisation-wide suppressions                       |
...
Global suppress rules apply to all scanners. Per-scanner sections (`[presidio]`, `[gitleaks]`, `[semgrep]`, `[sonarqube]`) apply only to that engine. Matching is by exact `rule_id` or by reference prefix (e.g. `CWE-798` suppresses any rule whose references start with `CWE-798`).
...
`src/server.py` exposes five MCP tools to VS Code Copilot Chat or any MCP-compatible client:
...
| File                          | Purpose                                                                   |
| ----------------------------- | ------------------------------------------------------------------------- |
| `src/config/regulations.yaml` | Regulation catalogue (UK GDPR, PCI DSS, PSR 2017) with article references |
| `src/config/suppress.txt`     | Global suppression rules (global + per-scanner sections)                  |
| `src/config/gitleaks.toml`    | Custom Gitleaks rule overrides                                            |
| `src/remediation/rules.yaml`  | 50+ category definitions (severity, fix_steps, references)                |
...
| **Network use is explicit**    | The built-in PII scanner is local. First-run binary/package downloads and Semgrep rule retrieval may use the network; SonarCloud is opt-in and sends analysis data to the hosted service. |
...
```
src/cli.py                  — CLI entry point (Typer)
